...
vector<string> parser(const string& instruction) {
    ...
    while (getline(ss, temp_word, ' ')) { // Tokenize by space
        ...
        // Check if the token is a label (at the beginning of the line or part of an instruction)
        if (col_start != string::npos && (col_start == 0 || temp_word.back() == ':')) {
            string label = temp_word.substr(0, col_start);
            label_map[label] = instruction_index2;
        }
