// assembler target address calculation
/* Work done:

Calculating immediate values(branch target address for jumps/branches)

work need to be  done:
handling 2 cases
1) instruction and label are on same line
2)instruction and label are on different lines and  empty lines between them


Assumption:
Weak detection for invalid instruction 
needs to be fixed
...
Myfile.seekg(0);  // Seek to the beginning of the file
    ...
    while (getline(Myfile, text)) {
        ...
        if (find(R_operations.begin(), R_operations.end(), operation) != R_operations.end()) {
            ...
            int newimm=(instruction_index-label_map[key])*2;
